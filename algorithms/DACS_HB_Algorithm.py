...
class DACS_HB_Algorithm:
    ...
    def run(self, B_list):
        ...
        results = []
        for T in range(1000):  # 因为这里是要得到准确答案，所以固定跑1w次
            B = B_list[T % len(B_list)].copy()
        ...
        print('最终cost为: ', anp.sum(self.cost_function(self.X, self.Z, self.p, self.b)))

        # 再来一次得到收敛图
        self.X_best = self.X
        self.initialize(B, grad_f_sigma)  # 初始化状态变量
        results = []
        for T in range(self.iter_num):  # 因为这里是要得到准确答案，所以固定跑1w次
            B = B_list[T % len(B_list)].copy()
            self.update_all_params(B, grad_f_x, grad_f_sigma, grad_phi_x)  # 更新所有参数
            sum_res = self.calculate_residual()  # 计算残差矩阵
            results.append(sum_res)
    ...
    def initialize(self, B, grad_f_sigma):
        ...
        self.P = anp.zeros((n, d))
        self.W = anp.zeros((n, d))
        self.Z = anp.zeros((n, d))
        self.XX = self.X.copy()
    ...
    def update_all_params(self, B, grad_f_x, grad_f_sigma, grad_phi_x):
        ...
        self.X_nxt = self.X - self.alpha * (grad_f_1 + grad_phi_x(self.X) * self.P) + self.vec * (self.X - self.XX)
...
if __name__ == "__main__":
    ...
    algorithm = DACS_HB_Algorithm(config)
    B = anp.array([  # 示例邻接矩阵
        [1/2, 0, 0, 1/2, 1/2],
        [1/2, 1/2, 0, 0, 0],
        [0, 1/2, 1/3, 0, 0],
